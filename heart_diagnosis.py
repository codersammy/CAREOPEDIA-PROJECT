...
class heart(tkinter.Tk):
    def __init__(self):
        def detection():
            ...
            parkinsons_data = pd.read_csv('heart.csv')
            ...
            X = heart_data.drop(columns=['status'], axis=1)
            Y = heart_data['status']
            ...
            try:
                input_data = (e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14)
                ...
                if prediction[0] == 0:
                    ...
                    sql = "UPDATE user SET heart_diseases ='NO' WHERE mr_no = '"+e14+"'"
                    mycursor.execute(sql)
                    mydb.commit()
                else:
                    messagebox.showinfo("Diseases report", "The person have Heart Disease")
                    print('The person have Heart Disease')
                    sql = "UPDATE user SET heart_diseases ='YES' WHERE mr_no = '"+e14+"' "
                    mycursor.execute(sql)
                    mydb.commit()
